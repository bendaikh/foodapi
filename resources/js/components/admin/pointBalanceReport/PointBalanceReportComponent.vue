        ...
        <div class="col-12">
            <div class="db-card db-tab-div active">
                <div class="db-card-header border-none">
                    <h3 class="db-card-title">{{ $t('menu.point_balance_report') }}</h3>
                    <div class="db-card-filter">
                        <TableLimitComponent :method="list" :search="props.search" :page="paginationPage" />
                        <FilterComponent @click.prevent="handleSlide('point-balance-filter')" />
                ...
                <div class="table-filter-div" id="point-balance-filter">
                ...
                <div class="db-table-responsive">
                    <table class="db-table stripe" id="print" :dir="direction">
                        ...
                        <tbody class="db-table-body" v-if="pointBalanceReports.length > 0">
                            <tr class="db-table-body-tr" v-for="user in pointBalanceReports" :key="user">
                                <td class="db-table-body-td">{{ user.name }}</td>
                                <td class="db-table-body-td">{{ user.email }}</td>
                                <td class="db-table-body-td">{{ user.phone ? user.country_code + '' + user.phone : '' }}</td>
                                <td class="db-table-body-td">{{ user.points }}</td>
...
export default {
    name: "PointBalanceReportComponent",
    components: {
        BreadcrumbComponent,
        TableLimitComponent,
        PaginationSMBox,
    ...
    data() {
        return {
            ...
            printObj: {
                id: "print",
                popTitle: this.$t('menu.point_balance_report')
            },
            props: {
                search: {
                    paginate: 1,
    ...
    computed: {
        pointBalanceReports: function () {
            return this.$store.getters['pointBalanceReport/lists'];
        },
        pagination: function () {
            return this.$store.getters['pointBalanceReport/pagination'];
        },
        paginationPage: function () {
            return this.$store.getters['pointBalanceReport/page'];
        },
        roles: function () {
    ...
    methods: {
        ...
        list: function (page = 1) {
            ...
            this.$store.dispatch('pointBalanceReport/lists', this.props.search).then(res => {
        ...
        xls: function () {
            this.loading.isActive = true;
            this.$store.dispatch('pointBalanceReport/export', this.props.search).then(res => {
                this.loading.isActive = false;
                const blob = new Blob([res.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const link = document.createElement('a');
                link.href = URL.createObjectURL(blob);
                link.download = this.$t("menu.point_balance_report");
                link.click();
