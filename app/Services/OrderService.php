...
class OrderService
{
    ...
    public function userOrder(PaginateRequest $request, User $user)
    {
        try {
            ...
            return Order::with('transaction', 'orderItems', 'branch', 'user')->where('active', Ask::YES)->where('order_type', "!=", OrderType::POS)->where(function ($query) use ($requests, $user) {
    ...
    public function deliveredOrder(PaginateRequest $request, User $user)
    {
        try {
            ...
            return Order::where('delivery_boy_id', $user->id)->where('active', Ask::YES)->where('order_type', "!=", OrderType::POS)->where(
    ...
    public function deliveryBoyOrder(PaginateRequest $request)
    {
        try {
            ...
            return Order::where('order_type', "!=", OrderType::POS)->where('active', Ask::YES)->where('delivery_boy_id', Auth::user()->id)->where(
                function ($query) use ($requests) {
                    foreach ($requests as $key => $request) {
                        if (in_array($key, $this->orderFilter)) {
                            $query->where($key, 'like', '%' . $request . '%');
    ...
    public function changeStatus(Order $order, $auth = false, OrderStatusRequest $request): Order|array
    {
        try {
            if ($auth) {
                ...
                if ($order->status == OrderStatus::PENDING &&  $request->status == OrderStatus::ACCEPT && 
